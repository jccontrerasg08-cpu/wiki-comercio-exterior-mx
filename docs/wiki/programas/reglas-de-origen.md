---
title: "Reglas de origen"
description: "Reglas de origen: criterios para determinar origen preferencial y sustentar beneficios arancelarios bajo el tratado aplicable."
---

# Reglas de origen

## Qué es

Las reglas de origen dicen cuándo un bien es “originario” de una parte del tratado (cambio de clasificación, contenido regional, procesos específicos). Sin origen no hay preferencia.

## Cómo aplica en México

El exportador/productor documenta el origen; el importador lo declara. Una regla incumplida convierte el arancel preferencial en arancel general más posibles recargos.

## Fuentes oficiales

- [T-MEC](https://www.gob.mx/t-mec)
- [SNICE LIGIE](https://www.snice.gob.mx/cs/avi/snice/ligie.info22.html)

Catálogo local: `docs/catalog/catalog.md`.

## Ver también

[TLC y T-MEC](tlc-tmec.md) · [TIGIE y NICO](../clasificacion/tigie-nico.md) · [Pedimento y RGCE](../aduana/pedimento-rgce.md)

> No es asesoría legal. Corrobora contra SIDOF, Diputados o SAT.
