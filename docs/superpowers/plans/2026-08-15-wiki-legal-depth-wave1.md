...
- [ ] **Step 2: Run the focused test in CI/PR and confirm it fails only because the approved content is absent.**
- [ ] **Step 3: Keep the test semantic rather than line-number based so editorial improvements do not create unnecessary churn.**
...
- [ ] **Step 1:** Replace the Padrón absolute with the general-rule/exceptions formulation tied to RGCE 1.3.1, 1.3.5, 1.3.6 and Annexes 7, 8 and 9.
- [ ] **Step 2:** Explain cuotas compensatorias as legally separate measures whose applicability depends on the exact resolution, including product scope, origin/exporter/producer/value/vigency/exceptions, not merely fraction + country.
- [ ] **Step 3:** Clarify that preferential treatment can lower IGI, while cuotas compensatorias are separate additional obligations when applicable.
- [ ] **Step 4:** Clarify Incoterms: allocation of costs/risks/clearance obligations can affect valuation inputs, but Incoterms do not determine Mexican customs procedure, legal importer, tariff or pedimento filer by themselves.
- [ ] **Step 5:** Ground documentary-credit material in ICC UCP 600 and distinguish bank document examination from customs compliance.
- [ ] **Step 6:** Reframe VUCEM references in light of the 4-May-2026 Ventanilla Única decree.
- [ ] **Step 7:** Replace `Siempre (despacho definitivo)` with a conditional document matrix and explicit dependence on regime, merchandise, transport and legal assumption.
- [ ] **Step 8:** Convert `Cambios 2026` into a dated timeline including the 31-Mar article 159 bis lineamientos, 23-Apr TIGIE/PROSEC decree, 4-May Ventanilla Única decree, 14-May 1st RMRGCE and 20-May annex modifications.
...
- [ ] **Step 1:** Explain transaction value as the primary method subject to legal conditions and adjustments.
- [ ] **Step 2:** Explain incrementables/non-incrementables, relationship between parties and when method 1 is not acceptable.
- [ ] **Step 3:** Describe the sequence of identical, similar, deductive, computed and fallback methods without presenting the summary as the binding text.
- [ ] **Step 4:** Add a worked numerical example clearly labeled pedagogical.
- [ ] **Step 5:** Separate Incoterm allocation from legal customs-valuation adjustments.
- [ ] **Step 6:** Add MV purpose, responsible actor, evidence and VUCEM relationship on the new page.
...
- [ ] **Step 1:** Explain the 2026 Ventanilla Única legal transition and distinguish the platform/service layer from the customs authority and legal effects of each filing.
- [ ] **Step 2:** Explain agent vs agency, patent/authorization, responsibility and the 159 bis Council process without treating the private customs agent as ANAM.
- [ ] **Step 3:** Add an end-to-end dispatch flow: classify, RRNA, value, documents, pedimento, payment/validation, activation, automated-selection/review where applicable, release and post-entry evidence.
- [ ] **Step 4:** Add a PAMA/infringement overview that distinguishes inspection findings, precautionary seizure and administrative procedure, with explicit instruction to consult the current Ley Aduanera for grounds and deadlines.
- [ ] **Step 5:** Add the pages to MkDocs navigation and the wiki map with correct Spanish accents.
...
- [ ] **Step 1:** Register the 31-Mar-2026 lineamientos source with publication/effective dates and the appropriate legal instrument relationship.
- [ ] **Step 2:** Register the 4-May-2026 Ventanilla Única decree with publication/effective dates and instrument relationship.
- [ ] **Step 3:** Add page metadata for every new wiki page, using `reviewed/current` only where this change actually reviews the cited official sources through 15-Aug-2026.
- [ ] **Step 4:** Update existing affected page metadata source lists where the new sources materially support the public explanation.
- [ ] **Step 5:** Regenerate catalog and coverage outputs using the repository scripts; do not hand-edit generated semantics.
...
- [ ] **Step 1:** Run focused legal-depth tests.
- [ ] **Step 2:** Run all repository tests.
- [ ] **Step 3:** Run repository integrity, catalog drift, page metadata, coverage policy/drift, temporal graph and RAG evaluation.
- [ ] **Step 4:** Run strict MkDocs build and legacy route verification.
